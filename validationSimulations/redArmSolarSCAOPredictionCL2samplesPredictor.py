...
for atm_name, draws in atm_cases.items():
    ...
    for use_vibrations in [False, True]:
        ...
        gain = 0.4
        decay = 0.9999
        modal_cmd = torch.zeros((controller.reconstructor[0].shape[0],1), dtype=torch.float64)
        ...
        for i in range(nIterations):
            ...
            if predictor.ready():
                prediction = torch.as_tensor(predictor.predict(), dtype=torch.float64).unsqueeze(1)
