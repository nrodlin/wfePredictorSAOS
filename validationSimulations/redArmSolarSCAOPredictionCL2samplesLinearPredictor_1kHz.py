...
res_dir = os.path.join(user_home, 'simulations', 'results', 'cl_2samplesLinearPrediction_1kHz')
...
for atm_name, draws in atm_cases.items():
    ...
    res_file_name = f"res_2samplesLinearPredictor_1kHz_{vibr_label}_{atm_name}_{draw_name}.h5"
    ...
    for i in range(nIterations):
        ...
        delayed_slopes = scao_light_path_list[0].slopes_1D.copy()
        predictor.push(delayed_slopes)

        if predictor.ready():
            prediction = torch.as_tensor(predictor.predict(), dtype=torch.float64, device=device).unsqueeze(1)
            modal_error = (-1)*controller.reconstructor[0]@prediction
            modal_cmd = gain * modal_error + decay * modal_cmd
            zonal_cmd = controller.modal_basis[0]@modal_cmd
            asm.updateDMShape(zonal_cmd)
        else:
            asm.updateDMShape(torch.zeros((controller.modal_basis[0].shape[0], 1), dtype=torch.float64, device=device))
