...
def main():
    ...
    res_dir = os.path.join(base_dir, 'results', 'cl_sin_cerrar')
    ...
    for atm_name, draws in cases_to_run.items():
        ...
        for draw_name, kwargs in draws_to_run.items():
            ...
            for use_vibrations in vibr_options:
                cfg_idx, total_cfg, vibr_label = get_config_info(args.sensor, 120, use_vibrations, atm_name, draw_name, args.test)
                logger.info("=" * 95)
                logger.info(f">>> [Config {cfg_idx}/{total_cfg}] ({cfg_idx/total_cfg*100:.1f}%) | Sensor: {args.sensor}x{args.sensor} | Mode: CL Sin Cerrar | Atm: {atm_name} | Draw: {draw_name} | Vibr: {vibr_label}")
                logger.info("=" * 95)
