            ...
            for use_vibrations in vibr_options:
                ...
                for i in range(args.n_iterations):
                    ...
                    if (i + 1) % 250 == 0 or (i + 1) == args.n_iterations or i == 0:
                        slopes_rms = float(np.std(scao_light_path_list[0].slopes_1D))
                        ...
                        logger.info(f"  [Iter {i+1:4d}/{args.n_iterations}] | Slopes RMS: {slopes_rms:.4f} px | PSF Peak: {psf_peak:8.2e} | Status: [{stability}]")
